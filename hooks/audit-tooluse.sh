#!/usr/bin/env bash
# PostToolUse → audit.jsonl writer (backend-developer plugin, v1.0.0+).
...
# with actor "backend-developer:hook:audit-tooluse".
#
# ADVISORY ROW: backend-developer agents run as subagents under the company-workflow
...
# backend-developer runs standalone (no company-workflow), these rows stand alone.
#
...
ROW=$(printf '%s' "$PAYLOAD" | jq -c \
  --arg ts "$(date -u +%FT%TZ)" \
  --arg actor "backend-developer:hook:audit-tooluse" \
  --arg kind "$KIND" '
  {
    ts: $ts,
    actor: $actor,
...
if [ "$SELF_TEST" -eq 1 ]; then
  printf '%s\n' "$ROW" | jq -e '.metadata.dedupe_key == "sess_test:toolu_test" and .metadata.duration_ms == 42 and .metadata.effort == "medium" and .metadata.advisory == true and .actor == "backend-developer:hook:audit-tooluse"' >/dev/null \
