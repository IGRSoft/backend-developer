# backend-developer hooks

Plugin-scoped hook scripts (added v1.0.0). They give backend-developer agents
their own audit trail and pre-compaction checkpoint when the plugin runs
**standalone** — and degrade cleanly to *advisory* rows when backend-developer
...
backend-developer specialists are spawned **as subagents under an orchestrating plugin**, whose own hooks (`hooks/audit-tooluse.sh`,
...
- The orchestrator's `audit-dedup` hook keeps the **orchestrator** row authoritative
  ...
  `actor: "backend-developer:hook:*"` when `dedupe_key` collides.

`audit-subagent.sh` **never** touches `state.json` — the three-layer state
merge (`state-merge.sh`) stays orchestrator-owned. backend-developer only reads
