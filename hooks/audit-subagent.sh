#!/usr/bin/env bash
# SubagentStop → audit.jsonl writer (backend-developer plugin, v1.0.0+).
# Emits a `subagent_stopped` row with actor "backend-developer:hook:audit-subagent".
#
# ADVISORY ROW: backend-developer specialists run as subagents under an orchestrating plugin,
...
read_stdin() {
  if [ "$SELF_TEST" -eq 1 ]; then
    printf '%s' '{"agent_type":"backend-developer:node-developer","agent_id":"agt_test","session_id":"sess_test","duration_ms":12345,"parent_agent_id":"agt_parent"}'
  else
    cat
  fi
}
...
ROW=$(printf '%s' "$PAYLOAD" | jq -c \
  ...
  {
    ts: $ts,
    actor: "backend-developer:hook:audit-subagent",
...
if [ "$SELF_TEST" -eq 1 ]; then
  printf '%s\n' "$ROW" | jq -e '
    ...
    and .subject == "backend-developer:node-developer"
    ...
    and .actor == "backend-developer:hook:audit-subagent"
  ' >/dev/null \
