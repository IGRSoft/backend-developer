...
**Runtime-contract clause (append verbatim to every stack reviewer prompt below).** The same twelve-factor defects appear in every stack, so they are stated once here rather than repeated per language:

> Also check the twelve-factor runtime contract: per-deploy config read from a committed config file or selected by branching on an environment name (`NODE_ENV`/`SPRING_PROFILES_ACTIVE`/`RAILS_ENV`) instead of arriving as validated env vars; state that must outlive a request held in process memory or on local disk (session maps, sticky sessions, uploads or generated files on the container filesystem); a hardcoded listen port, or a port bound to loopback only; no `SIGTERM` handler draining in-flight work; migrations or backfills executed on application boot; logfiles written or rotated in-process instead of unbuffered JSON to stdout. Anchor: `skill: containerization`.
...
- Prompt: "Read-only review of the JVM files: {file_list}. Review for: Bean Validation on DTOs, authorization boundaries (method/object security, BOLA on repository fetches), @Transactional correctness (self-invocation proxy bypass, propagation, exception-rollback rules), N+1 across JPA/Hibernate associations, idempotency, error handling (@ControllerAdvice, leaked stack traces), and reactive/blocking misuse (block() on reactive threads). Also apply the runtime-contract clause stated above this list. Do NOT edit any file. Return findings as a list of `{file, line, category, severity (P0-P3), why, fix, confidence}`. If there are no material issues, say so directly."
