...
| No health/readiness discipline | Missing or lying health endpoints, no graceful shutdown | Bad deploys ship silently |
...
- Prompt: "Read-only operational debt inventory over these paths: {file_list} (stacks present: {stacks}). Load `skills/tooling/observability/SKILL.md`. Inventory: config sprawl (settings read from several places, duplicated defaults, undocumented env vars, secrets committed in config files — report the location, never the secret value), observability gaps (request paths with no structured log, trace span, or metric; correlation IDs not propagated across service calls; errors swallowed without a signal), and health/shutdown discipline (missing or non-representative readiness/liveness endpoints, no graceful shutdown). Do NOT edit any file. Return findings as `{file, line, item, evidence, operational_impact}`. If a category is clean, say so in one line."
- Error: Non-critical — note the gap and continue.
