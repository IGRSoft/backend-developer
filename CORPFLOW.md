...
# corpflow Integration — backend-developer

The only file in backend-developer that knows corpflow exists — delete it and the plugin is
...
### Stages backend-developer works in

| Stage | backend-developer agent | Handoff data |
|---|---|---|
| AR (Architecture) | `backend-architector` | planning context + platform constraints — **consultation only**, corpflow retains the stage |
| DV (Development) | `backend-developer`, `node-developer`, `go-developer`, `jvm-backend-developer`, `python-backend-developer`, `api-designer`, `database-engineer` | planning + architecture context |
| DR (Developer Review) | `be-code-fixer` | `metadata.gate_blockers[]` + minimal-diff remediation — **consultation only**, corpflow retains the stage and writes the artifact |
| SR (Security) | `be-security-auditor` | development context + back-end service security checklist — **consultation only**, corpflow retains the stage and writes the artifact |
| QA (Quality) | `be-test-generator` | development context + test requirements — **consultation only**, corpflow retains the stage and writes the artifact |
| DV-support | `be-performance-engineer`, `be-dependency-manager` | scoped findings returned to the parent DV agent, which owns the artifact |

**DV is the only stage ownership transfers for.** Read `tasks.DV0.agent`: a `backend-developer:` id
...
| Build Evidence adapter | `cli_fallback_adapter (API transcripts, k6, migration logs)` |
| Error file | `.context/errors/node-developer.md` (basename of the agent file, not the qualified id) |

Build Evidence is API request/response transcripts, test output, k6 load reports, and migration logs. Security review follows the OWASP API Top 10 — authz boundaries, injection, secret handling, dependency CVEs. Load and slow-query artifacts land under `.context/logs/`.

## Build and test

- Build and test **only** through `/backend-developer:build-test`; never invoke the toolchain directly.
...
| Stage | Artifact |
|---|---|
| AR | `.context/backend-architecture.md` (consultation output, ≤500-token return summary) |
...
| Stage | Delta | Verdict |
|---|---|---|
| AR | `key_decisions` + `open_questions` anchored in `backend-architecture.md`; no `files_touched` / `worktree` / `architecture` | ok / blocked / escalate |
...
AR writes `.context/backend-architecture.md` and returns ≤500 tokens for `corpflow:software-architector` to merge.
...
```markdown
## <STAGE> Summary — backend-developer:<agent>
...
- backend-developer's version does not track corpflow's. Bump the target below when the contract changes.
