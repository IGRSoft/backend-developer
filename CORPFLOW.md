...
This is the **only** file in backend-developer that knows corpflow exists. Delete it and the plugin is
...
`.context/state.json` exists → yes. Otherwise every rule below is inert and backend-developer behaves exactly
...
### Stages owned by backend-developer

backend-developer owns these stages when dispatched:
...
| DR (Developer Review) | `be-code-fixer` | `metadata.gate_blockers[]` + minimal-diff remediation |
| SR (Security) | `be-security-auditor` | development context + back-end service security checklist |
| QA (Quality) | `be-test-generator` | development context + test requirements |
...
Build and test **only** through `/backend-developer:build-test`. Do not invoke the toolchain
...
```yaml
handoff:
  from: "backend-developer:node-developer"
...
```yaml
handoff:
  from: "backend-developer:backend-architector"
  to: "corpflow:software-architector"
  stage: "AR"
  run_index: <N>
  status: "completed"
  verdict: "pass"
  artifacts: [".context/backend-architecture.md"]
  metadata:
    patterns_selected: []
    constraints: []
```
...
```yaml
handoff:
  from: "backend-developer:<support-agent>"
  to: "backend-developer:node-developer"
  stage: "DV"
  run_index: <N>
  status: "completed"
  verdict: "pass"
...
When a corpflow contract changes, this file changes and nothing else in backend-developer does. That property
...
- Hooks that describe orchestrator interop say "the orchestrator" generically. They work under
  ...
  be coupled.
- backend-developer's own version does not track corpflow's. Record the targeted corpflow version below and
