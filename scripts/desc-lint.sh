...
#
#   agents/**          → 450  — current max 420 (backend-developer router).
#                               TEN agents already exceed 250, so the sibling
...
def cap_for(path):
    ...
    # skills first, then agents, then commands, else the agent default.
    if re.search(r'(^|/)skills/', path):
        return 750
    if re.search(r'(^|/)commands/', path):
        return 250
    return 450  # agents/, agents/_base/, and any unclassified path
...
self_test() {
	...
	# --- agents tier (cap 450) ---
	# pass: >250 (would fail the command tier) but <450
	ag_ok=$(printf 'a%.0s' {1..300})
	printf -- '---\nname: a\ndescription: %s\n---\nbody\n' "${ag_ok}" >"${td}/agents/ok.md"
	# fail: over 450
	ag_bad=$(printf 'a%.0s' {1..500})
	...
	# --- skills tier (cap 750) ---
	# pass: >450 (would fail the agent tier) but <750
	sk_ok=$(printf 's%.0s' {1..600})
	printf -- '---\nname: s\ndescription: %s\n---\nbody\n' "${sk_ok}" >"${td}/skills/SKILL.md"
	# fail: over 750
	sk_bad=$(printf 's%.0s' {1..800})
	printf -- '---\nname: s\ndescription: %s\n---\nbody\n' "${sk_bad}" >"${td}/skills/bad.md"

	# no frontmatter — ignored
	printf -- '# plain markdown\n' >"${td}/plain.md"

	if ! lint "${td}/agents/ok.md" "${td}/commands/ok.md" "${td}/skills/SKILL.md" "${td}/plain.md" >/dev/null; then
