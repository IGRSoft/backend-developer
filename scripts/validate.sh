...
ALLOWED_PREFIX_RE='^(backend-developer|system-developer|corpflow|security-scanning|debugging-toolkit|general-purpose)'
	...
	while IFS= read -r f; do
		...
		while IFS= read -r st; do
			...
			if [[ ! "${st}" =~ ${ALLOWED_PREFIX_RE} ]]; then
				err "${rel}" "subagent_type '${st}' has a disallowed plugin prefix" "use backend-developer:/system-developer:/corpflow:/security-scanning:/debugging-toolkit:/general-purpose:"
				continue
			fi
