#!/usr/bin/env bash
#
# validate.sh — release-gate validator for the backend-developer plugin.
#
...
for m in "${PLUGIN_MANIFEST}" "${MARKET_MANIFEST}"; do
	if [[ ! -f "${m}" ]]; then
		err "${m}" "manifest missing" "create ${m} per plan §7"
		continue
	fi
...
# plugin's own cache copies (anything under a backend-developer path) so a
...
if [[ -d agents ]]; then
	while IFS= read -r f; do
		...
		[[ -n "${ext}" ]] && err "${rel}" "agent name '${aname}' collides with ${ext}" "rename to a be-prefixed or otherwise unique name"
	done < <(find agents -type f -name '*.md')
fi
...
ALLOWED_PREFIX_RE='^(backend-developer|system-developer|company-workflow|security-scanning|debugging-toolkit|general-purpose)'
	...
	while IFS= read -r f; do
		...
		while IFS= read -r st; do
			...
			if [[ ! "${st}" =~ ${ALLOWED_PREFIX_RE} ]]; then
				err "${rel}" "subagent_type '${st}' has a disallowed plugin prefix" "use backend-developer:/system-developer:/company-workflow:/security-scanning:/debugging-toolkit:/general-purpose:"
				continue
			fi
			# Own-plugin targets must exist as agent files.
			case "${st}" in
				backend-developer:*)
					target="${st#backend-developer:}"
