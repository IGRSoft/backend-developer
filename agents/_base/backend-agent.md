...
Backend agents participate in the company-workflow 11-stage workflow system (v4.0.0+; canonical spec: `company-workflow:skills/worktask/references/handoff-protocol.md`).
...
Technical-lead (`company-workflow:technical-lead`) reviews DV output against backend-specific criteria (API-contract adherence, error handling, transaction correctness, N+1 queries, idempotency, auth boundaries, migration safety). Backend agents support DR by:
