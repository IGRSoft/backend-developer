...
When `.context/state.json` exists, this agent is inside an igrsoft workflow. Follow `_base/backend-agent.md § Workflow Stage Participation § DV Stage` for the contract; the router-specific steps:
